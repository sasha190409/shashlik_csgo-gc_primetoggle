...
{
    // Используем абсолютный путь и флаг, который заставляет искать зависимости
    // в папке загружаемой библиотеки, не изменяя глобальный PATH.
    HMODULE module = LoadLibraryExW(absoluteModulePath, nullptr, LOAD_WITH_ALTERED_SEARCH_PATH);
    if (!module)
    {
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nShowCmd)
{
    ...
    // Отрезаем имя исполняемого файла
    wchar_t *slash = wcsrchr(baseDir, '\\');
    if (!slash)
    {
        slash = baseDir; // fallback
    }
    *slash = L'\0';

    // НЕ модифицируем переменную PATH – это частая причина ложных срабатываний.
    // Вместо этого полагаемся на LOAD_WITH_ALTERED_SEARCH_PATH в LoadLibraryExW.
